...
int main(int argc, char *argv[]) {
  ...
  printf(
      "If you have any make errors or warnings then you will receive a "
      "straight 0.\n");
...
int test_mount_unmount() {
  ...
  if (rc == 1) {
    printf(
        "failed: mount should fail on an already mounted system but it "
        "succeeded.\n");
    return 0;
  }
  ...
  if (rc == 1) {
    printf(
        "failed: unmount should fail on an already unmounted system but it "
        "succeeded.\n");
    return 0;
  }
...
int test_read_invalid_parameters() {
  ...
  if (mdadm_read(addr, SIZE, buf1) != -1) {
    printf(
        "failed: read should fail if it goes beyond the end of the linear "
  ...
  if (mdadm_read(0, sizeof(buf2), buf2) != -1) {
    printf(
        "failed: read should fail on larger than 1024-byte I/O sizes but it "
        "did not.\n");
    return 0;
  }
  ...
  if (mdadm_read(0, SIZE, NULL) != -1) {
    printf(
        "failed: read should fail when passed a NULL pointer and non-zero "
